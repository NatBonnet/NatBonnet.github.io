...
therm <- read.csv(here::here("posts", "stats_model", "data", "hyperthermal-tolerance.csv"), 
...
n <- 1000 # Sample size
beta0  <- 0.5 # Intercept
beta_1 <- -0.2 # Effect of body mass on response
phi <- 2 # Gamma shape parameter

# Generate data for predictors
pred1<- rnorm(n, mean = 0, sd = 1)
...
sim_data <- data.frame(resp, pred1)

sim_fit <- glm(resp ~ pred1,
