...
## Addressing geospatial complexity in determining suitable areas for aquaculture
...
This analysis used available oceanographic data to stack multiple environmental variables and identify where their suitable ranges overlap, allowing us to quantify and map overall aquaculture suitability within the U.S. West Coast Exclusive Economic Zone (EEZ) for a given species. The ultimate goal was to develop a reproducible method for assessing the aquaculture potential of different species across EEZ areas.
...
The first step in a workflow with multiple spatial objects should be to check if coordinate reference systems (CRS) match between datasets.
...
if (crs(depth_crop_res) == crs(mean_sst) &
all(res(depth_crop_res)) == all(res(mean_sst)) &
ext(depth_crop_res) == ext(mean_sst)){
  print("Rasters can stack")
} else{
  warning("Rasters do not share all necessary attributes")
}
```
...
eez_table <- eez_zone_areas %>% 
  select(rgn, area) %>% 
  st_drop_geometry() %>% slice_max(order_by = area, n = 1) %>% 
