...
**Did the United Kingdom's 2001 Climate Change Programme (CCP) reduce per capita CO₂ emissions?**
...
-   In this study, we investigate and run SCM for 3 different donor pools. This positions the UK's CCP to be evaluated against a wide pool of donor countries by pre-treatment carbon emissions:
    ...
    -   \(2\) High income counties (HIC) with similar economic conditions to the UK (OECD).

    -   \(3\) High and upper-middle income (HIC and UMC) counties with similar economic conditions to the UK (OECD).
...
| `data_OECD` | Carbon emissions from countries with similar economic conditions to the UK. Includes 23 countries. |
| `data_OECD_HIC` | Carbon emissions from high income counties with similar economic conditions to the UK. Includes 33 countries. |
| `data_OECD_HIC_UMC` | Carbon emissions from high and upper-middle income counties with similar economic conditions to the UK. Includes 51 countries. |
...
source(here("posts", "carbon_policy", "R", "load_rdata_files.R"))
...
-   **Pink lines***:* The treated unit's (UK) actual gap which is the actual UK's CO2 per capita minus the synthetic UK's CO2 per capita.
...
# Modify tidysynth function to filter out negligible weights
...
plot_weights_filtered(scm_data_OECD_HIC_UMC) +
...
However, it is important to note the MSPE results contradict the placebo tests results. The MPSE ratio might be inflated due to the nearly-perfect pre-treatment fit which is displayed in the [Synthetic Control Generation]. With an extremely small pre-treatment fit, modest post-treatment deviations produce a large MSPE ratio. Therefore, the UK might not diverge dramatically in post-treatment and just has a near-zero pre-treatment gap.

```{r mspe-top-donors}
#| code-fold: true

# Looking at a specific MSPE criteria
scm_data_OECD_HIC_UMC %>%
  grab_significance() %>%
...
Lastly, we shall look at the Balance Table. Ideally, a properly optimized synthetic control model will have balance, meaning equal or near-equal values across the different predictors between the real United Kingdom and the synthetic version. We use a balance table to compare pre-treatment predictor values in the synthetic unit, the real unit, and the donor pool. In this scenario, the balance table compares pre-treatment predictor values for the real United Kingdom, the synthetic United Kingdom, and the donor pool average.
...
balance_OECD %>% 
  kable(digits = 4, 
      ...
      col.names = c("Year", "United Kingdom", "Synthetic UK", "Donor Sample")) %>%
  kable_styling(bootstrap_options = c("striped", "condensed"))
...
![Lepissier and Mildenberger's final plot.](images/figure_4.png)
...
In summary, using different means to achieve a SCM method, we have replicated a similar synthetic control model, showed accurate fit between our model and the real UK, and produced results very close to those found within the paper. Nevertheless, we did have some slight variation in the robustness checks.
